...
class TestParagraphs:
    ...
    def test_inline_code_plain_text(self):
...
class TestCLI:
    def test_stdin(self):
        result = subprocess.run(
            [sys.executable, str(Path(__file__).parent.parent / "scripts" / "md2mowen.py"), "-"],
            input="Hello **world**",
            capture_output=True,
            text=True,
        )
    ...
    def test_pretty_flag(self, tmp_path):
        ...
        result = subprocess.run(
            [sys.executable, str(Path(__file__).parent.parent / "scripts" / "md2mowen.py"), str(md_file), "--pretty"],
            capture_output=True,
            text=True,
        )
        assert result.returncode == 0
